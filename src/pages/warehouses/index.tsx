...
const Warehouses = () => {
    ...
    const fetchWarehouses = async () => {
        setLoading(true);
        try {
            const response = await dispatch(getPagedWarehouses({ filters: { pageSize, pageIndex, searchQuery, sort: -1, status } }));
            ...
            {import.meta.env.MODE === "development" ? (
                <>
                    ...
                    <Box display="flex" p="lg" className="items-center justify-between">
                        <Text size="lg" fw={500}>Warehouses</Text>
                    </Box>
                        ...
                        <Group w={{ lg: "60%", sm: "100%" }}>
                            <TextInput
                                ...
                                rightSection={searchQuery ? (
                                    <IconX className="cursor-pointer" onClick={() => { setSearchQuery(""); setPageIndex(1); }} size={14} />
                                ) : ""}
                    ...
                    <Box visibleFrom="lg" mx="lg" my="lg" className="overflow-x-auto">
                        <Table striped highlightOnHover withTableBorder withColumnBorders>
                            <Table.Thead>
                                <Table.Tr>
                                    <Table.Th style={{ width: "20%" }}>Warehouse ID</Table.Th>
                                        ...
                                        <Table.Tr key={i}>
                                            ...
                                            <Table.Td>
                                                <Button
                                                    ...
                                                    onClick={() => navigate(`/app/warehouses/view-warehouse/${c._id}`)}
                                                >
                                                    View
                                                </Button>
                                            </Table.Td>
                                    ...
                                    <Table.Tr>
                                        <Table.Td colSpan={4} className="text-center">
                                            <div className="flex justify-center items-center">
                                                <IconDatabaseOff color="red" size="24" className="mr-2" />
                                                No data available
                                            </div>
                                        </Table.Td>
                                    </Table.Tr>
                        ...
                        {warehouses?.length ? (
                            warehouses.map((c: any, i: number) => (
                                <Card key={i} shadow="sm" withBorder mx="xs" my="lg">
                                    <Text className="font-semibold">Warehouse ID: {c.id}</Text>
                ...
                <Alert title="Notice" color="yellow" m="md">
                    <Text c="yellow">This Page Under Development</Text>
                </Alert>
            )}
        </>
    );
